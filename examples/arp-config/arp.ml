...
module ARP : sig
  ...
  let send_request tpa =
    ...
    let sha = read Os_var.local_mac in
    (* Source protocol address, pick one of our IP addresses *)
    let spa = match read bound_ips with
      | [] -> ipv4_blank
      | h :: _ -> h
    in
    (read Os_var.network_out) Lwt_io.stdout (string_of_arp { op=`Request; tha; sha; tpa; spa })
  ...
  let query ?(timeout = 10.) eth =
    ...
    with Not_found ->
      ...
      H.add cache eth tu;
      Lwt.ignore_result (send_request eth);
      Lwt.ignore_result (
        fork_and_join
          (Lwt_stream.next
  ...
  let input arp = match arp.op with
    |`Request ->
      ...
      if List.mem req_ipv4 (read bound_ips) then begin
        ...
        (read Os_var.network_out) Lwt_io.stdout (string_of_arp {op=`Reply; sha; tha; spa; tpa})
      end else Lwt.return ()
    |`Reply ->
